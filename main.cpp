...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_02_03";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::DragFloat3("point" , &point.center.x , 0.01f);
		ImGui::DragFloat("point.radius" , &point.radius , 0.01f);

		if (IsAABBtoSphereCollision(aabb1, point)) {
			color = RED;
		} else {
			color = WHITE;
		}
		...
		DrawAABB(aabb1 , projectionMatrix , viewportMatrix , color);
		DrawSphere(point , projectionMatrix , viewportMatrix , WHITE);
